...
| Malicious terminal escapes or WebView resource exhaustion | xterm window controls disabled; OSC 8 activation blocked; no OSC 52 clipboard integration; input IPC is capped at 256 KiB, frontend backlog at 4 MiB, and backend command queues at 64 entries; scrollback is capped at 20,000 lines; backend accepts canonical UUID session IDs and at most 64 concurrent terminals | Visual spoofing and high-rate output can still consume CPU and transient buffers |
| Parser/route denial of service | Include depth/cycle checks; 8 MiB, 1024-file, and per-glob match budgets; non-recursive polynomial-time wildcard matching; `known_hosts` rejects symlinks, is capped at 16 MiB, and serializes in-process writes; synchronous config/trust-store I/O runs on the blocking pool; unit/property tests; continuous fuzzing | Inputs within those budgets can still create substantial parser and UI work; other local processes can still edit trust files concurrently |
...
| SFTP path traversal or unsafe overwrite | Native picker scopes local roots; Rust rejects absolute/parent/NUL paths and symlink escape; upload validates the opened descriptor before remote mutation; same-target transfers are mutually exclusive; remote names, paths, listings, queue, and completed history are bounded; temporary-file rename and backup rollback protect existing files | A malicious server may misreport metadata or fail operations; a same-authority local process can still race intermediate path components; recursive directory transfer and resume are intentionally unavailable |
...
| Session log secret leakage or viewer memory exhaustion | Logging is per-target opt-in and output-only; auth prompts bypass it; path is picker-scoped; read/write opens reject symlinks and non-files; new Unix logs are `0600`; rotation is bounded; viewer streams with line/result limits and at most four concurrent searches | Remote/child output and normal PTY echo can still contain secrets; directory permissions and retention remain the operator's responsibility |
