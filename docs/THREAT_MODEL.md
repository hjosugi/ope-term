...
| Parser/route denial of service | Include depth/cycle checks; 8 MiB, 1024-file, and per-glob match budgets; non-recursive polynomial-time wildcard matching; `known_hosts` is a regular file capped at 16 MiB; synchronous config/trust-store I/O runs on the blocking pool; unit/property tests; continuous fuzzing | Inputs within those budgets can still create substantial parser and UI work |
